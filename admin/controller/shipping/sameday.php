...
class ControllerShippingSameday extends Controller
{
    private $error = array();
    ...
    {
        $this->load->model('shipping/sameday');
        $this->model_shipping_sameday->install();
        $this->load->model('setting/setting');
        $this->model_setting_setting->editSetting("sameday", array("sameday_sync_until_ts" => time()));
        $this->model_setting_setting->editSetting("sameday", array("sameday_sync_lockers_ts" => 0));
    }

    public function uninstall()
    {
        $this->load->model('shipping/sameday');
        $this->model_shipping_sameday->uninstall();
    }

    public function index()
    {
        $this->load->language('shipping/sameday');
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST' && $this->validate()) {
            ...
            // $this->request->post["sameday_sync_until_ts"] = $this->getConfig('sameday_sync_until_ts');
            // $this->request->post["sameday_sync_lockers_ts"] = $this->getConfig('sameday_sync_lockers_ts');
            // $this->request->post["sameday_testing"] = $this->getConfig('sameday_testing');
            // $this->request->post["sameday_host_country"] = $this->getConfig('sameday_host_country') ?? $this->samedayHelper::API_HOST_LOCALE_RO;

            // if (null !== $this->testing && null !== $this->hostCountry) {
            //     $this->request->post["sameday_testing"] = $this->testing;
            //     $this->request->post["sameday_host_country"] = $this->hostCountry;
            // }

            // $this->model_setting_setting->editSetting("sameday", $this->request->post);

            // $this->session->data['error_success'] = $this->language->get('text_success');

            // $this->response->redirect($this->url->link('shipping/sameday', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        $data['breadcrumbs'] = array(
            ...
            array(
                ...
                'href' => $this->url->link('shipping/sameday', $this->addToken(), true)
            )
        );

        $data['statuses'] = $this->getStatuses();
        $data['action'] = $this->url->link('shipping/sameday', $this->addToken(), true);
        $data['cancel'] = $this->url->link($this->getRouteExtension(), $this->addToken(array('type' => 'shipping')), true);
        $data['services'] = $this->model_shipping_sameday->getServices($this->getConfig('sameday_testing'));
        $data['service_refresh'] = $this->url->link('shipping/sameday/serviceRefresh', $this->addToken(), true);
        $data['pickupPoints'] = $this->model_shipping_sameday->getPickupPoints($this->getConfig('sameday_testing'));
        $data['lockers'] = $this->model_shipping_sameday->getLockers($this->getConfig('sameday_testing'));
        $data['pickupPoint_refresh'] = $this->url->link('shipping/sameday/pickupPointRefresh', $this->addToken(), true);
        $data['lockers_refresh'] = $this->url->link('shipping/sameday/lockersRefresh', $this->addToken(), true);
        $data['tax_classes'] = $this->model_localisation_tax_class->getTaxClasses();
        $data['geo_zones'] = $this->model_localisation_geo_zone->getGeoZones();
        $data['service_links'] = array_map(
            function ($service) {
                return $this->url->link('shipping/sameday/service', $this->addToken(array('id' => $service['id'])), true);
            },
            $data['services']
        );
        ...
        $this->response->setOutput($this->load->view('shipping/sameday.tpl', $data));
    }
    ...
    public function serviceRefresh()
    {
        ...
        $this->model_shipping_sameday->ensureSamedayServiceCodeColumn();
        ...
        do {
            ...
            try {
                ...
                $this->response->redirect($this->url->link('shipping/sameday', $this->addToken(), true));
            }

            foreach ($services->getServices() as $serviceObject) {
                $service = $this->model_shipping_sameday->getServiceSameday($serviceObject->getId(), $this->isTesting());
                if (!$service) {
                    // Service not found, add it.
                    $this->model_shipping_sameday->addService($serviceObject, $this->isTesting());
                } elseif ($service['sameday_code'] != $serviceObject->getCode()) {
                    $this->model_shipping_sameday->updateServiceCode($service['id'], $serviceObject->getCode());
                }
        ...
        $localServices = array_map(
            ...
            $this->model_shipping_sameday->getServices($this->isTesting())
        );
        ...
        foreach ($localServices as $localService) {
            if (!in_array($localService['sameday_id'], $remoteServices)) {
                $this->model_shipping_sameday->deleteService($localService['id']);
            }
        }
    ...
    public function pickupPointRefresh()
    {
        ...
        do {
            ...
            foreach ($pickUpPoints->getPickupPoints() as $pickupPointObject) {
                $pickupPoint = $this->model_shipping_sameday->getPickupPointSameday($pickupPointObject->getId(), $this->isTesting());
                if (!$pickupPoint) {
                    // Pickup point not found, add it.
                    $this->model_shipping_sameday->addPickupPoint($pickupPointObject, $this->isTesting());
                } else {
                    $this->model_shipping_sameday->updatePickupPoint($pickupPointObject, $pickupPoint['id']);
                }
        ...
        $localPickupPoints = array_map(
            ...
            $this->model_shipping_sameday->getPickupPoints($this->isTesting())
        );
        ...
        foreach ($localPickupPoints as $localPickupPoint) {
            if (!in_array($localPickupPoint['sameday_id'], $remotePickupPoints)) {
                $this->model_shipping_sameday->deletePickupPoint($localPickupPoint['id']);
            }
        }
    ...
    public function lockersRefresh()
    {
        ...
        foreach ($lockers as $lockerObject) {
            $locker = $this->model_shipping_sameday->getLockerSameday($lockerObject->getId(), $this->isTesting());
            if (!$locker) {
                $this->model_shipping_sameday->addLocker($lockerObject, $this->isTesting());
            } else {
                $this->model_shipping_sameday->updateLocker($lockerObject, $locker['id']);
            }
        ...
        $localLockers = array_map(
            ...
            $this->model_shipping_sameday->getLockers($this->isTesting())
        );
        ...
        foreach ($localLockers as $localLocker) {
            if (!in_array($localLocker['sameday_id'], $remoteLockers)) {
                $this->model_shipping_sameday->deleteLocker($localLocker['id']);
            }
        }
    ...
    {
        $this->load->model('shipping/sameday');
        $service = $this->model_shipping_sameday->getService($this->request->get['id']);
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST' && $this->validatePermissions()) {
            $this->model_shipping_sameday->updateService($service['id'], $this->request->post);

            $this->session->data['error_success'] = $this->language->get('text_success');

            $this->response->redirect($this->url->link('shipping/sameday/service', $this->addToken(array('id' => $service['id'])), true));
        }
        ...
        $data['breadcrumbs'] = array(
            ...
            array(
                ...
                'href' => $this->url->link('shipping/sameday/service', $this->addToken(array('id' => $service['id'])), true)
            )
        );

        $data['action'] = $this->url->link('shipping/sameday/service', $this->addToken(array('id' => $service['id'])), true);
        $data['cancel'] = $this->url->link('shipping/sameday', $this->addToken(), true);
        ...
        $this->response->setOutput($this->load->view('shipping/sameday_service.tpl', $data));
    }
    ...
    public function info($orderInfo)
    {
        ...
        $data = array(
            ...
            'buttonAddAwbLink' => $this->url->link('shipping/sameday/addAwb', $this->addToken(array('order_id' => $orderInfo['order_id'])), true),
            'buttonShowAwbPdf' => $this->url->link('shipping/sameday/showAsPdf', $this->addToken(array('order_id' => $orderInfo['order_id'])), true),
            'buttonShowAwbHistory' => $this->url->link('shipping/sameday/showAwbHistory', $this->addToken(array('order_id' => $orderInfo['order_id'])), true),
            'buttonDeleteAwbLink' => $this->url->link('shipping/sameday/deleteAwb', $this->addToken(array('order_id' => $orderInfo['order_id'])), true)
        );

        $awb = $this->model_shipping_sameday->getAwbForOrderId($orderInfo['order_id']);
    ...
    public function addAwb()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST' && $this->validateFormBeforeAwbGeneration()) {
            ...
            $service = $this->model_shipping_sameday->getServiceSameday($params['sameday_service'], $this->isTesting());
            ...
            if (isset($awb)) {
                $this->model_shipping_sameday->saveAwb(array(
                ...
                $this->model_shipping_sameday->updateShippingMethodAfterPostAwb($orderInfo['order_id'], $service);
        ...
        $data['counties'] = $this->model_shipping_sameday->getCounties();
        ...
        $data['breadcrumbs'] = array(
            ...
            array(
                ...
                'href' => $this->url->link('shipping/sameday/createAwb', $this->addToken(array('order_id' => $orderInfo['order_id'])), true)
            )
        );
        ...
        * Actions
        */
        $data['estimate_cost_href'] = $this->url->link('shipping/sameday/estimateCost');
        $data['action'] = $this->url->link('shipping/sameday/addAwb', $this->addToken(array('order_id' => $orderInfo['order_id'])), true);
        ...
        $this->response->setOutput($this->load->view('shipping/sameday_add_awb.tpl', $data));
    }
    ...
    public function showAwbHistory()
    {
        ...
        $this->load->model('shipping/sameday');
        $awb = $this->model_shipping_sameday->getAwbForOrderId($this->request->get['order_id']);
        ...
        */
        ...
        $data['breadcrumbs'] = array(
            ...
            array(
                'text' => 'AWB',
                'href' => $this->url->link('shipping/sameday/showAwbStatus', $this->addToken(array('order_id' => $awb['order_id'])), true)
            )
        );
        ...
        if(empty($_SERVER['HTTP_X_REQUESTED_WITH']) || strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) !== 'xmlhttprequest') {
            ...
            $this->response->setOutput($this->load->view('shipping/sameday_awb_history_status.tpl', $data));
        ...
        foreach ($parcels as $parcel) {
            ...
            $this->model_shipping_sameday->refreshPackageHistory(
                $awb['order_id'],
                $parcel->getAwbNumber(),
        ...
        $data['packages'] = $this->model_shipping_sameday->getPackagesForOrderId($awb['order_id']);

        $this->response->setOutput($this->load->view('shipping/sameday_awb_history_status_refresh.tpl', $data));
    ...
    public function deleteAwb()
    {
        ...
        $awb = $this->model_shipping_sameday->getAwbForOrderId($orderId);
        $sameday = new Sameday($this->samedayHelper->initClient());

        if ($awb) {
            try {
                $sameday->deleteAwb(new \Sameday\Requests\SamedayDeleteAwbRequest($awb['awb_number']));
                $this->model_shipping_sameday->deleteAwb($awb['awb_number']);
            } catch (\Exception $e) { }
        }
    ...
    {
        if (!$this->user->hasPermission('modify', 'shipping/sameday')) {
    ...
    private function validate()
    {
        ...
        if ($this->request->post["sameday_username"] !== $username) {
            // Username changed.
            $username = $this->request->post["sameday_username"];
            $needLogin = true;
        }

        $password = $this->getConfig('sameday_password');
        if (!empty($this->request->post["sameday_password"])) {
            // Password updated.
            $password = $this->request->post["sameday_password"];
            $needLogin = true;
        } else {
            $this->request->post["sameday_password"] = $password;
        }
    ...
    private function buildRequest(array $keys)
    {
        $entries = array();
        foreach ($keys as $key) {
            $entries["sameday_$key"] = $this->request->post["sameday_$key"] ?? $this->getConfig("sameday_$key");
        }
